...
**sanctscan** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `sanctscan` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
sanctscan ... --format json | cognis-connect emit --to stix   # STIX 2.1 bundle
sanctscan ... --format json | cognis-connect emit --to slack --url $URL --token $TOK   # Slack
sanctscan ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `sanctscan` outputs:

```python
from cognis_connect import normalize, stix
findings = [normalize(rec, source="sanctscan") for rec in records]   # records = your JSON output
