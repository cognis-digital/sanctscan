"""Native cognis-connect emit for sanctscan — forward findings to any platform.

Maps sanctscan's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    sanctscan ... --format json | sanctscan-emit --to stix
    sanctscan-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = rec.get('name', '') + ' - ' + rec.get('list', '')
        out['severity'] = 'medium'
        out['type'] = 'sanction'
        out['description'] = rec.get('description', '')
        out['tags'] = [rec.get('list', ''), rec.get('country', '')]
