...
How **sanctscan** fits the wider Cognis suite — a composable set of defensive, analytical
tools that all run on your own hardware and speak plain **JSON** (or an OpenAI-compatible
**`/v1`**). `sanctscan` lives in the **General OSINT / security tool** cluster.
...
| from | relation | to |
|---|---|---|
| `sanctscan` | JSON findings compose with | [`awesome-cognis`](https://github.com/cognis-digital/awesome-cognis) |
| `sanctscan` | AI add-ins are served `/v1` by | [`edgemesh`](https://github.com/cognis-digital/edgemesh) (your fleet) |
| `sanctscan` | findings can be narrated / reasoned over by | [`humind`](https://github.com/cognis-digital/humind) -> [`agentlex`](https://github.com/cognis-digital/agentlex) |
| `sanctscan` | export to intel formats via | [`stixgen`](https://github.com/cognis-digital/stixgen) (STIX) / [`attackmap`](https://github.com/cognis-digital/attackmap) (ATT&CK) |

## Composition patterns

Everything reads/writes JSON, so tools chain with ordinary pipes; nothing leaves the box.

**1 — chain within the general osint / security tool cluster.** `sanctscan` output feeds the next tool:
```bash
sanctscan ... --format json > out.json          # this tool's findings (see `sanctscan --help`)
...
sanctscan ...                                         # vision / reasoning add-ins light up
```

**3 — export findings to your SOC's formats.**
```bash
sanctscan ... --format json | stixgen from-json > bundle.stix.json   # STIX 2.1
sanctscan ... --format json | attackmap map > attack.json            # ATT&CK techniques
```

**4 — narrate through cognition + agents.** `humind` extracts salience from `sanctscan`'s
output; `agentlex` holds it as KB facts and fires Horn rules to escalate. `sanctscan` slots
